...
interface Deduction {
  ...
  /**
   * 演出の重さ（省略時は 'normal'）。
   * 'heavy': fraud系・組織の根幹を揺るがす真実——amber より強い演出（色・テキスト・効果音）で
   * 「見抜いた手応えの段違い感」を表現する。
   */
  weight?: 'normal' | 'heavy'
  /**
   * 縦糸アーク専用のトーン（省略可。省略時は通常の推理フレーム）。
   * 'arc': 不正暴露アークなど物語の重大な転換点——「推理」バッジ・amber 系から
   * rose/slate 系の空気感へ切り替え、「——異変」バッジと「核心を突け」案内文で
   * 「今回は違う」合図を UI フレーム側から出す。既存 13 件の variant 未指定イベントは完全に変わらない。
   */
  variant?: 'arc'
}
 ...
 *  プランニングで上位を「スプリント予測」としてスプリントバックログに引く。 */
export interface BacklogItem {
  ...
  /** プランニングで「How（実行計画）」へ分解できる作業項目（任意）。これがある PBI だけ splitIntoSbi で
   *  分割でき、スプリント予測上で `${id}#1`, `${id}#2`… の作業項目（サブタスク）に展開される。
   *  ＝Scrum Guide 2020 のスプリントプランニング「PBI を作業可能な単位へ分解する」の表現。
   *  各 estimate の合計＝親 PBI の estimate（不変条件・テストで検証）。割るかはプレイヤー判断（割らない手も有効）。 */
  split?: { title: string; estimate: number }[]
}
