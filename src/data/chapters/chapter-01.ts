...
export const FINALE_EPILOGUES: Record<'expose' | 'exposeWeak' | 'complicit' | 'coopted', Epilogue> = localizeDeep({
  expose: {
    ...
    title: '告発したFDE',
    reflection:
      '固めた“動かぬ証拠”を、あなたは外に出した。循環取引の輪は白日の下にさらされ、ジェネリック電機は揺れる。倍力装置の実証に付いた{{産学官連携}}の{{補助金}}にも、調査の手が伸びるだろう。買収された外様のカルゴ物流の行く末も不透明になり、現場の雇用に痛みも走るだろう。それでも——あなたは“見栄えの数字”でなく“本物の数字”の側に立った。正しさは、いつもほろ苦い。だが、沈黙していた現場が、初めて本当のことを言える場所になった。',
  },
  exposeWeak: {
...
export const FINALE: { prompt: string; choices: FinaleChoice[] } = localizeDeep({
  prompt:
    '現場をIT化して実数を可視化したら、“導入済み”のはずの{{フィジカルAI}}機材が、倉庫のどこにも無かった。掘るほどに、グループの数字の裏に影が差す——カルゴ物流を踏み台に、同じ機材を書類の上だけで巡らせる、架空の循環取引の影。倉庫でお披露目された“倍力装置”の実証機も、{{産学官連携}}の{{補助金}}という錦の御旗を背負い、書類の上を一周するたびに売上と研究実績の双方を生んでいた。本物の成果を出そうとして、本物でない数字にたどり着いてしまった。さあ、どうする。（“動かぬ証拠”を固めていれば、告発は通る。）',
  choices: [
