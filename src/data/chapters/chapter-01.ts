...
export const FINALE_EPILOGUES: Record<'expose' | 'exposeWeak' | 'complicit' | 'coopted', Epilogue> = localizeDeep({
  expose: {
    ...
    title: '告発したFDE',
    reflection:
      '固めた“動かぬ証拠”を、あなたは外に出した。循環取引の輪は白日の下にさらされ、ジェネリック電機は揺れる。買収された外様のカルゴ物流の行く末も不透明になり、現場の雇用に痛みも走るだろう。それでも——あなたは“見栄えの数字”でなく“本物の数字”の側に立った。正しさは、いつもほろ苦い。だが、沈黙していた現場が、初めて本当のことを言える場所になった。',
  },
  exposeWeak: {
