...
export const FINALE: { prompt: string; choices: FinaleChoice[] } = {
  prompt:
    '現場をIT化して実数を可視化したら、“導入済み”のはずのフィジカルAI機材が、倉庫のどこにも無かった。掘るほどに、グループの数字の裏に影が差す——カルゴ物流を踏み台に、同じ機材を書類の上だけで巡らせる、架空の循環取引の影。本物の成果を出そうとして、本物でない数字にたどり着いてしまった。さあ、どうする。（“動かぬ証拠”を固めていれば、告発は通る。）',
  choices: [
