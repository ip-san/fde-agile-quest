...
export const SPRINT2_EVENTS: GameEvent[] = [
  ...
  // ── レトロ ──
  {
    ...
    choices: [
      ...
      {
        // genbaTrust を立てつつ capacity レバー: 引き出した暗黙知を、人のレビューで丁寧に受け止める手を厚くする。
        // 標準化が進むほどレビューに回す項目が増える＝レビューが追いつかず素通しが起きている周回に効く。
        id: 'b',
        label: '信頼を築き、引き出したノウハウを人の{{レビュー}}で受け止める量を厚くする',
        effects: { insight: 1, culture: 1 },
        resultText:
          '遠回りだが、田淵さんが少しだけ口を開いた。「まあ、ここだけの話な…」。隠されたノウハウへの細い道が見えた。久遠が問う。「もし出てくる暗黙知をレビューが取りこぼしてるなら、人がコードと向き合う手を一つ増やすか？」。次スプリントから{{レビュー}}に割く手を厚くすると決めた。レビューの量が増え、多く確かめて多く届けられる。（一つに絞って仕上がりを上げる道は今回見送り＝機会コスト／号令で速く進める信頼+も取り逃す）',
        setsFlag: 'genbaTrust',
        retroLever: 'capacity',
      },
      {
