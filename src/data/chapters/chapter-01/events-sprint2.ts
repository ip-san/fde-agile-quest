...
export const SPRINT2_EVENTS: GameEvent[] = [
  ...
  // ── プランニング ──
  {
    ...
    title: 'KPIを定める',
    narrative:
      '前スプリントで{{完成の定義}}まで届かなかった分は、いったん{{プロダクトバックログ}}に戻っている。今回の{{フォーキャスト}}は、そこから組み直す——その前に、このスプリントの成功を測る{{KPI}}を1つ決める。何を「良くなった」とみなすかで、この先すべてが変わる。',
    hearingOptions: [
  ...
  // ── レトロ ──
  {
    ...
    title: '振り返り——核心の壁',
    narrative:
      '{{レトロスペクティブ}}。動く分は出せたが、{{完成の定義}}まで届かず{{キャリーオーバー}}した項目もある——なぜ終わりきらなかったか。理由をたどると、全員が同じ壁に行き着いた。自動化の鍵は、田淵さんや山田さん、橋本さんの“勘”——例外処理の暗黙知だ。だが現場は、自分の仕事や価値が消えるのを恐れて、それを言葉にしない。隠している。この主軸を、どう攻略する？',
    choices: [
      {
        id: 'a',
  ...
  // 上にしか乗らない——という学びの芯。視察団来訪（visit）とは別物の“見学”として両立。
  {
    ...
    title: '試作ロボットのお披露目',
    narrative:
      'ジェネリック電機が、自社開発の{{フィジカルAI}}の試作機をカルゴ物流の倉庫に“持ち込んで”、グループ向けにお披露目していた。導入したわけではない——お披露目が終われば持ち帰られる、文字どおりのデモ機だ。倉庫の一角だけ床に真新しい誘導ラインが引かれ、白い搬送ロボットがその線の上だけをするすると往復する。脇に据えた産業用アームが箱をつまんでは番重に並べ、{{バーコード}}を読んで棚番を照合する。前に人が立つと、速度を落として止まる。そして一角の中央には、お披露目の“花形”として人型の二足歩行ロボットが立ち、来賓へぎこちなく手を振ってみせる——まだ実作業はせず、見せるための一体だ。スーツの一団が「これが物流の未来です」と頷く。だが視線を5メートル横にずらせば、そこは田淵さんの世界だ。手書き台帳と、線を無視して通路を横切るフォークリフト。ロボットが触れているのは、整えられたデモ用の棚だけ——実際の出荷の8割は、いまも田淵さんの勘と紙で回っている。',
    hearingOptions: [
