...
export const SPRINT2_EVENTS: GameEvent[] = [
    ...
    choices: [
      {
        ...
        resultText:
          '締切は守った。結城さんには「今は速さを優先し、後で必ず返す」と一言握ったうえで動くものを当てた。安堵（信頼+）。{{フィードバックループ}}が短くなった代わりに、持続可能なコードで積み上げる足場は今回あきらめた。借りた{{技術的負債}}のしわ寄せは、当面チームへの無理として残る（巻き込み−）。返済は早いほどいい。',
      },
      {
        id: 'b',
  ...
  // 帳簿だけの機材／s3 循環取引・実証報告）が何の話か腑に落ちる＝後の物語の土台になる。
  {
    ...
    title: '試作ロボットのお披露目',
    narrative:
      'ジェネリック電機が、自社開発の{{フィジカルAI}}の試作機をカルゴ物流の倉庫に“持ち込んで”、グループ向けにお披露目していた。導入したわけではない。お披露目が終われば持ち帰られる、文字どおりのデモ機だ。倉庫の一角だけ床に真新しい誘導ラインが引かれ、白い搬送ロボットが線の上だけをするすると往復する。脇に据えた産業用アームが箱をつまんでは折りたたみコンテナに整然と並べ、搬送ライン上の固定スキャナが{{バーコード}}を読んで棚番を照合する。前に人が立つと、速度を落として止まる。一角の中央には“花形”として人型の二足歩行ロボットが立つが、まだ実作業はしない。見せるための一体だ。スーツの一団が「これが物流の未来です」と頷く。だが視線を5メートル横へずらせば、そこは田淵さんの世界だ。手書き台帳と、線を無視して通路を横切るフォークリフト。ロボットが触れているのは、整えられたデモ用の棚だけ。実際の出荷の8割は、いまも田淵さんの勘と紙で回っている。',
    hearingOptions: [
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1 },
        resultText:
          'よく見れば、あのデモ機は“棚番と在庫データが寸分違わず正しく、走る線も整っている”という前提の上で初めて動いていた。線をなぞる{{AGV}}だろうと、線が無くても走る{{AMR}}だろうと、現場のIT化・データ化という地面が無ければお披露目で止まる。{{フィジカルAI}}は魔法じゃない。「派手な実物より、まず正しいデータと動線」。その順番を構造として腹に落とした。（上に同調すれば得られた信頼+は取り逃す）',
      },
      {
        id: 'c',
