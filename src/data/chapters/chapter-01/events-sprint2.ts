  ...
  // ── デイリー ──
  {
    ...
    choices: [
      ...
      },
      {
        ...
        resultText:
          '「これで…大丈夫なの？」。粗い叩き台に、結城さんは不安顔（未完成を見せて信頼−）。その日のうちに田淵さんが触った。{{フィードバックループ}}が、回り始める。',
        seedId: 'photo-input',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        resultText:
          '締切は守った。結城さんは安堵（信頼+）。動くものがすぐ現場に当たり、{{フィードバックループ}}が短くなった。しわ寄せは借りた{{技術的負債}}として当面チームが背負う（巻き込み−）。返済は早いほどいい。',
      },
      {
        id: 'b',
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        repo: { coverage: 20 },
        resultText:
          '評価なき生成は祈り。今度は{{評価基準}}をAIに書かせ、人が検証した。在庫ズレが止まり、現場の信頼が戻った（実害を止めて信頼+）。トークンは食う。だがこれが、AIとの正しい組み方だ。',
        tokenCost: 250,
      },
    ],
  },
  ...
  // ── レビュー ──
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 2 },
        resultText:
          '黙って後ろに立つ。田淵さんが写真をパシャ、画面を一瞥、次の棚へ。「お、これなら楽だわ」。アンケートの数字には、この一瞬は載らない。（報告映えの信頼+は取り逃す）',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        effects: { trust: 1, insight: -1 },
        resultText:
          '「お、君も分かってるな」とスーツ陣が破顔し、横で結城さんも上への面目が立って表情を緩める（進捗が見えて信頼+）。乗ること自体は間違いじゃない——グループが本気で張る技術に前のめりで手を挙げるのは、立ち位置として理にかなう。ためらえば実証案件ごと他拠点へ行く。だが順番を間違えた。うちの棚番も在庫データもまだガタガタで、その熱量を足元のデータ整備へ向け替えないと、入れても線の上で空回りするだけだ（足元から目が逸れて現場理解−）。',
        warn: true,
      },
      {
        id: 'b',
        ...
        effects: { insight: 1 },
        resultText:
          'よく見れば、あのデモ機は“棚番と在庫データが寸分違わず正しく、走る線も整っている”という前提の上で初めて動いていた。決められた線の上を走るだけの機体（{{AGV}}）か、自分で経路を判断して走る機体（{{AMR}}）か——どちらにせよ、現場のIT化・データ化という地面が無ければお披露目で止まる。{{フィジカルAI}}は魔法じゃない。「派手な実物より、まず正しいデータと動線」。その順番を構造として腹に落とした。（上に同調すれば得られた信頼+は取り逃す）',
      },
      {
        id: 'c',
  ...
  // ただしトレードオフは残す（把握に賭けたぶん、目に見える機能はまだ薄い）。
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1, culture: 1 },
        resultText:
          '「夜勤明けは“数えた数”じゃなく“引き継いだ言葉”が消えるのが怖い」——沈黙していた理由が、田淵さんの口から要件になった。把握を急がず積んだ一次情報が、作るべきものの芯を太くする。立てた{{スプリントゴール}}が、ここで実を結んだ。（一気に作り込んで見せる信頼+は取り逃す＝把握に賭けた機会コスト）',
      },
    ],
  },
]
