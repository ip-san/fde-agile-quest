...
export const SPRINT2_EVENTS: GameEvent[] = [
  ...
  },
  {
    ...
    segment: 'trouble',
    title: '精度の頭打ち',
    narrative: 'AIの需要予測の的中率が92%で頭打ち。チームは「99%まで上げよう」と時間をかけたがる。',
    choices: [
      {
        id: 'a',
    ...
    choices: [
      {
        ...
        effects: { trust: 1, culture: -1 },
        resultText:
          'レトロで壁は見えた。だが解き方を現場に委ねず、専務へ上げて業務命令で押した。一声で話は速く進み、経営の覚えもいい（信頼+）。現場は渋々、表向きの手順だけを差し出す。いちばん大事な“例外”は、まだ誰の口からも出てこない。',
        warn: true,
        setsFlag: 'topDown',
      },
      {
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        repo: { coverage: 15 },
        resultText:
          '速さは活かしつつ、出力は人の目で受け入れる。見当違いの実装も、存在しないAPIをそれらしく呼ぶ{{ハルシネーション}}も、ここで食い止める。（“全部お任せ”の速さが持つ信頼+は取り逃す）',
      },
    ],
  },
  {
  ...
  // ── 買収の皮肉アーク（視察当日）。showcasePressure が立っていれば確実に出る ──
  {
    ...
    narrative:
      '親会社の試作ロボットが倉庫の一角でお披露目されている同じ時期、今度はジェネリック電機の視察団が本番の視察にやって来た。スーツの一団は“動くAI”や自動倉庫ロボットを期待している。だが目の前にあるのは、田淵さんの20年分の手書き台帳と、行き交うフォークリフトだ。視線が泳ぐ。「…で、{{フィジカルAI}}は、どちらに？」。',
    hearingOptions: [
  ...
  // 上にしか乗らない——という学びの芯。視察団来訪（visit）とは別物の“見学”として両立。
  {
    ...
    title: '試作ロボットのお披露目',
    narrative:
      'ジェネリック電機が、自社開発の{{フィジカルAI}}の試作機をカルゴ物流の倉庫に“持ち込んで”、グループ向けにお披露目していた。導入したわけではない。お披露目が終われば持ち帰られる、文字どおりのデモ機だ。倉庫の一角だけ床に真新しい誘導ラインが引かれ、白い搬送ロボットがその線の上だけをするすると往復する。線の上をなぞって走るこの{{AGV}}に対し、線が無くても自分で経路を判断して走る{{AMR}}という型もある——どちらにせよ、棚番と在庫データが正確という土台があって初めて動くのだが。脇に据えた産業用アームが箱をつまんでは折りたたみコンテナに整然と並べ、傍らの固定スキャナが{{バーコード}}を読んで棚番を照合する。前に人が立つと、速度を落として止まる。一角の中央には“花形”として人型の二足歩行ロボットが立つが、まだ実作業はしない。見せるための一体だ。スーツの一団が「これが物流の未来です」と頷く。だが視線を5メートル横へずらせば、そこは田淵さんの世界だ。手書き台帳と、線を無視して通路を横切るフォークリフト。ロボットが触れているのは、整えられたデモ用の棚だけ。実際の出荷の8割は、いまも田淵さんの勘と紙で回っている。',
    hearingOptions: [
