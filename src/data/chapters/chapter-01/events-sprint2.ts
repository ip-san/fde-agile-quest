  ...
  // ガードレール：借りた判断＝常に悪、にしない。選択bは「握った通り計画的に返す」報われる道（trust据え置き＝守れば損なわない）を残す（単一正解化の回避）。
  {
    ...
    choices: [
      ...
      },
      {
        ...
        resultText:
          '「後で返す」の“後で”を、今日にした。借りた所を名指しで直し、田淵さんが“こわごわ”だった一点に評価基準を足して、約束ぶんの返済を{{バックログ}}から消す。手を止めたぶん目先の進捗は止まって見えたが、結城さんは渋い顔をしなかった——「あの時“後で返す”って言ってたやつ、ちゃんと返したのか」。自分から握った期日を、その期日のうちに果たした。約束を守った相手は、信頼を減らさない（顧客に切った期日を守った＝据え置き）。早く返した負債は利息がつかず、次の改修も軽くなる。（“走り続けて見せる”側の信頼+は取り逃す＝早期返済に賭けた機会コスト）',
      },
    ],
  },
