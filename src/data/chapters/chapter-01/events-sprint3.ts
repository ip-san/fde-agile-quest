...
export const SPRINT3_EVENTS: GameEvent[] = [
...

  {
    ...
    title: '放置した基盤が、詰まる',
    narrative:
      '本番が伸びてきた途端、20年ものの{{レガシー}}基盤が悲鳴を上げた。朝会で「基盤の更新」を後回しにし続けた{{技術的負債}}のツケだ。今、土台から軋み始めている。',
    choices: [
      {
        id: 'a',
  ...
  // フラグ一致のバリアントを優先して出す＝決定論的に展開が変わる。
  {
    ...
    title: '最終レビュー：崩れるデモ',
    narrative:
      '最終{{スプリントレビュー}}。{{フォーキャスト}}した本番化を「{{完成の定義}}まで届いた」と見なし、郷田専務の前で自動化した{{ピッキング}}を本番データでデモする。だが——号令で集めた手順には、現場が隠した“例外”が抜けていた。完成と数えたはずのものが、実は終わっていなかった。イレギュラーな品が来た瞬間、自動仕分けが連鎖的に誤り、画面が{{欠品}}と誤出荷の山を映し出す。',
    hearingOptions: [
  ...
  },
  {
    ...
    narrative:
      '最終{{スプリントレビュー}}。{{フォーキャスト}}した本番化が、例外処理まで含めて{{完成の定義}}に届いた——今回は数えた分が、ちゃんと終わっている。自動化した{{ピッキング}}を本番データでデモする。田淵さんがこっそり教えてくれた“例外の見分け方”が組み込まれ、イレギュラーな品が来ても、システムは現場の勘そのままに正しく弾いた。郷田専務がうなる。',
    hearingOptions: [
      {
  ...
  },
  {
    ...
    title: '最終レビュー：成果を語る',
    narrative:
      '最終{{スプリントレビュー}}。{{フォーキャスト}}した分を{{完成の定義}}まで届いた{{インクリメント}}と、持ち越した分に仕分け、誤出荷率の推移とともに郷田専務に報告する。{{PoC}}止まりにせず本番で回り始めた、終わった分の成果だ。',
    hearingOptions: [
      {
  ...
  // 主軸の結末。topDown は挽回（建て直す/突き放す）、genbaTrust は文化定着。既定 s3-retro の前に置く。
  {
    ...
    title: '最後のレトロ：建て直せるか',
    narrative:
      'キャンペーン最後の{{レトロスペクティブ}}。トップダウンで押し切った自動化は、本番でつまずいた。隠された例外が漏れたからだ。「完成」と数えたものが{{完成の定義}}に届いておらず、結局いちばん肝心な分が{{キャリーオーバー}}になった。チームは沈み、現場との溝は深い。最後に、どう締める？',
    hearingOptions: [
  ...
  },
  {
    ...
    title: '最後のレトロ：根を張る文化',
    narrative:
      'キャンペーン最後の{{レトロスペクティブ}}。信頼を積み、現場と一緒に隠れたノウハウを形にした自動化は、本番でも例外まで含めて回っている。コミットした分を例外処理まで含めて{{完成の定義}}まで届けきり、持ち越しを最小に締めた。田淵さんが言う。「正直、最初はあんたが仕事を奪いに来たと思ってた」。学びをどう残す？',
    hearingOptions: [
  ...
  },
  {
    ...
    title: '最後のレトロスペクティブ',
    narrative:
      'キャンペーン最後の{{レトロスペクティブ}}。{{フォーキャスト}}した分のうち何を{{完成の定義}}まで終わらせ、何を持ち越したか——3スプリント分を並べると、チームは口々に言う。「{{KPI}}を“誤出荷を減らす”に据えられるかどうかで、結果がここまで変わるとは。機能数を追っていたら、作ったものは全部やり直しだった」。この案件の学びをどう残す？',
    hearingOptions: [
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1, culture: 1 },
        resultText:
          '「自分が決める。責任は取る」。決まった瞬間、チームが動き出した。{{自己組織化}}したチームでも、最後に決める一点は要る。',
      },
    ],
  },
  {
  ...
  // ＝発注者を下げる痛みは現場を下げる痛みより重い（非対称）。trust摩擦は STORY §3 不変ルール：信頼−は顧客摩擦時のみ・理由を結果文に明記。
  {
    ...
    hearingTheme: 'team',
    hints: {
      po: '結城さんが「約束した数字は、いつ見せられるんだ」と詰めてきてる。電算室で受け止めて。',
      sm: '現場を優先したぶん、結城さんの見せ場が後回しになった。電算室で不安を聞いて。',
      dev: '結城さんが経営に詰められてるみたいです。電算室で、何を見せれば足りるか確かめましょう。',
    },
    title: '見せる数字が、まだ無い',
    narrative:
      '電算室に呼ばれた。結城さんの机に、経営向けの報告フォーマットが開いたまま固まっている。「現場の使い勝手、使い勝手って——こっちは経営に『在庫を減らす』『誤出荷を減らす』って数字で約束したんだ。その数字を見せる画面が、まだ一枚も無い」。前のスプリントで現場のゴールを通したぶん、結城さんが上に見せたい成果は、丸ごと後回しになっていた。声が、いつもより低い。',
    hearingOptions: [
