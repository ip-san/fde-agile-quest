...
export const SPRINT3_EVENTS: GameEvent[] = [
  ...
  // フラグ一致のバリアントを優先して出す＝決定論的に展開が変わる。
  {
    ...
    title: '最終レビュー：崩れるデモ',
    narrative:
      '最終{{スプリントレビュー}}。予測した本番化を「{{完成の定義}}まで届いた」と見なし、郷田専務の前で自動化した{{ピッキング}}を本番データでデモする。だが——号令で集めた手順には、現場が隠した“例外”が抜けていた。完成と数えたはずのものが、実は終わっていなかった。イレギュラーな品が来た瞬間、自動仕分けが連鎖的に誤り、画面が{{欠品}}と誤出荷の山を映し出す。',
    hearingOptions: [
  ...
  },
  {
    ...
    narrative:
      '最終{{スプリントレビュー}}。予測した本番化が、例外処理まで含めて{{完成の定義}}に届いた——今回は数えた分が、ちゃんと終わっている。自動化した{{ピッキング}}を本番データでデモする。田淵さんがこっそり教えてくれた“例外の見分け方”が組み込まれ、イレギュラーな品が来ても、システムは現場の勘そのままに正しく弾いた。郷田専務がうなる。',
    hearingOptions: [
      {
