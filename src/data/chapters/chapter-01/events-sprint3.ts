...
export const SPRINT3_EVENTS: GameEvent[] = [
    ...
    choices: [
      {
        ...
        effects: { trust: -1, culture: -1 },
        resultText:
          '「現場の習熟不足です」と言い繕った。事実とズレた釈明に郷田専務の信頼は揺らぎ（信頼−）、現場は「結局、俺たちのせいか」と完全に背を向けた（巻込−）。隠された例外は、隠されたままだ。',
        warn: true,
      },
      {
        id: 'b',
        ...
        effects: { insight: 1, trust: -1 },
        resultText:
          '失敗を経営の前でさらすのは痛い（粗い結果を見せて信頼−）。だが「あなた方の例外処理が要る。教えてください」と頭を下げた瞬間、田淵さんの眉がわずかに動いた。挙回の糸口は、誠実さの側にある。',
      },
    ],
  },
  {
    ...
    narrative:
      '自動化した{{ピッキング}}を本番データでデモする。山田さんがこっそり教えてくれた“例外の見分け方”が、ちゃんと組み込まれている。イレギュラーな品が来ても、システムは現場の勘そのままに正しく弾いた。郷田専務がうなる。',
    choices: [
      {
        id: 'a',
  ...
  // ── レトロ ──
  // 主軸の結末。topDown は挙回（建て直す/突き放す）、genbaTrust は文化定着。既定 s3-retro の前に置く。
  {
    id: 's3-retro-topdown',
    sprint: 3,
    ceremony: 'retro',
    ...
    choices: [
      {
        ...
        effects: { culture: -1 },
        resultText:
          '波風は立たなかった。だが「長いものに巻かれろ」が空気になり、現場の声がまた一つ死んだ。',
        warn: true,
      },
      {
        id: 'b',
        ...
        effects: { insight: 1, culture: 1 },
        resultText:
          '「出向、結構です」。感情でなく事実で受け返すと、課長は黙った。チームが「この人についていく」と背筋を伸ばした。',
      },
    ],
  },
]
