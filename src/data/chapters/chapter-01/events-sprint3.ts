...
export const SPRINT3_EVENTS: GameEvent[] = [
  ...
  // ── プランニング ──
  {
    ...
    title: '本番化の構え',
    narrative:
      '結城さんはいま、何かを言葉にしないまま席に座っている。その沈黙を横目に、第3スプリントが始まる。終わらなかった分が{{プロダクトバックログ}}に戻ってきた。並べ直して、今回の{{スプリント予測}}を上から組む。写真入力の叩き台は、もう現場の手に渡っている。{{PoC}}で終わらせるな。本番に根付かせる最後のスプリント。ゴールは？',
    hearingOptions: [
    ...
    choices: [
      {
        ...
        effects: { culture: -1 },
        resultText:
          '何かあれば、体が勝手に橋本さんの席へ向かう。「ほら、やっぱり俺だろ」。頼られた橋本さんは満更でもない顔だ。だが、その笑みの奥が少し疲れていることに、今日は誰も気づかない。',
        warn: true,
      },
      {
        id: 'b',
        ...
        effects: { culture: 1, insight: 1 },
        resultText:
          'つっかえながらも、橋本さん抜きで捌けた。一行見落として{{WMS}}を一本やり直し、彼がいた頃より今日は滞った。「もう俺、いらんかもな」。見守る橋本さんが寂しそうに笑う。必要とされた手応えが薄れる寂しさと、肩の荷が下りる解放感が、同時に滲んでいた。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { culture: 1, insight: 1 },
        resultText:
          '「これなら、自分たちで回せそうだ」。橋本さんも横で頷き、「俺の頭ん中、ここに書いときゃいいのか」と勘所を一つ書き足した。一人で抱えてきた彼が、初めて“次の誰か”へ手渡す側に回る。（寄り添いの信頼+は取り逃す）',
      },
    ],
  },
  {
