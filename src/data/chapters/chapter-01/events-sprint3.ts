...
export const SPRINT3_EVENTS: GameEvent[] = [
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1 },
        resultText:
          '数字で語れ——会計の帳尻と現場の実数を、間宮さんと静かに突き合わせた。だがこれは、一介のFDEが今この場で告発して済む話ではない。あまりに大きい。あなたは見たものを胸に刻み、記録だけを残した。この違和感は、まだ終わらない。（波風を立てない信頼+は取り逃す）',
        setsFlag: 'fraudCase',
      },
    ],
  },
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1, culture: 1 },
        resultText:
          '「契約を切るのはそちらのご判断です。ですが、事実は変わりません」。感情でなく数字で受け返すと、課長は黙った。契約も出向もちらつかせて揺るがないFDEに、現場が「この人についていく」と背筋を伸ばした。（事を荒立てない信頼+は取り逃す）',
      },
    ],
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1 },
        resultText:
          '伝票・在庫・入出庫のログが、一本の輪につながった。架空の循環取引の影——だがこれは、たった一人のFDEが今この場で告発して終わる話ではない。あまりに、大きすぎる。あなたは見たものを胸に刻み、静かに記録を残した。この違和感は、まだ終わらない。',
        setsFlag: 'fraudCase',
      },
    ],
  },
  ...
  // ── 買収の皮肉アーク（グループへの実証報告）。showcasePressure ゲート ──
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1, culture: 1 },
        resultText:
          '見栄えはしない報告だった。だが初めて、“本物の数字”が上に届いた。夢を語る前に、現実を一歩ずつ。それがFDEの仕事だと、結城さんも頷いた。（角を立てぬ信頼+は取り逃す）',
      },
    ],
  },
  ...
  // ── プランニング分岐の回収（S3で“自分が窓口”ゴールを選んだ周回だけ）──
  {
    ...
    title: '窓口が、あなたで止まる',
    narrative:
      'プランニングで「自分が運用の頼れる窓口であり続ける」をゴールに置いた。その通り、問い合わせはすべてあなたに集まる。だが——あなたが一日離れた途端、すべてが止まった。社内メンバーは指示待ちになり、組織は賢くならないまま。属人化というゴールのツケが、最終盤で跳ね返ってきた。',
    choices: [
      {
        id: 'a',
      ...
      },
      {
        ...
        effects: { culture: 1, insight: 1 },
        resultText:
          '見せ場は減る。だが、あなたが抜けても回る形へ一歩動いた。FDEの仕事は、組織を賢くするところまで。ゴールは終盤でも立て直せる。（抱え続ければ得た信頼+は取り逃す）',
      },
    ],
  },
]
