  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { trust: 1, culture: 1 },
        resultText:
          '「これ、俺が教えるんか」。田淵さんは渋い顔をしながら、どこか照れたように引き受けてくれた。現場は、よそ者の作った画面ではなく、二十年やってきた田淵さんの口から数字の見方を聞く。身内から学ぶぶん、画面が早く馴染んでいく（文化+）。「倉庫の人間が教える」という事実そのものが、現場の信頼を呼んだ（信頼+）。ただ、数字の読み方を自分の言葉で言語化して渡す——その勘所を掴む機会は田淵さんに委ねたぶん、自分の手元には残らなかった（洞察は取り逃す）。',
      },
    ],
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { insight: 1 },
        resultText:
          'いきなり書かせず、まずは読み取りだけ任せた。挙動を実運用で確かめ、信頼できると見てから書き込みを一段ずつ足していく。暴走の余地は段ごとに狭まる。だが完全自動化までの間、段ごとに切り戻しを設計し直す手間が要り、「もう全部任せられるのか」と急ぐ結城さんには進みが遅く映った（一気に見せる信頼+は取り逃す）。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        effects: { culture: 1 },
        resultText:
          '自分では決めず、一人ずつ「あなたの譲れない一点は」と引き出し、論点を黒板に並べた。皆が自分で並べた論点を見て、自分たちで結論に辿り着く。自分で決めたものは、決めた後の足が速い（自律性+）。だが全員の主張を整理し切るには時間がかかり、その日のうちに顧客へ示せる前進は遅れた（その場で着地させる信頼+は取り逃す）。論点が一点でぼやけているだけなら、方向を示して決めたほうが速い場面だった。',
      },
    ],
  },
  {
