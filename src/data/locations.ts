...
// 久遠（SM）＝問いを置く師。落ち着いて、答えより問いを残す。「答えは資料の外」。
const SM_LINES: Line[] = [
  (t, l) => `「${t}」……答えは資料の外にある。${l}で、自分の目で確かめておいで。`,
  (t, l) => `焦らなくていい。「${t}」が本当は何なのか、${l}でゆっくり見てきなさい。`,
  (t, l) => `「${t}」、どこかで流れが詰まっている気がするね。${l}でその一点を探してごらん。`,
  (t, l) => `ふむ。「${t}」は、放っておくほど絡まる類かもしれない。${l}を覗いてきなさい。`,
  (t, l) => `問いを一つ置こう。「${t}」で、いちばん困っているのは誰だ。${l}で確かめておいで。`,
  (t, l) => `急ぎの仕事ほど、足を運ぶ値打ちがある。「${t}」、${l}へ行ってきなさい。`,
]
...
const DEV_REPO_LINES: Line[] = [
  ...
  (t, l) => `白状すると「${t}」、AIに書かせた所が怪しいんです。${l}で読み直して計測を入れたい。`,
...
const SENSITIVE_LINES: Record<DailyRole, Line[]> = {
  ...
  sm: [
    (t, l) => `「${t}」、無理に急がなくていい。${l}で何が起きているか、丁寧に確かめておいで。`,
    (t, l) => `「${t}」、答えは資料の外だ。${l}で、人の話と記録に当たってきなさい。`,
    (t, l) => `「${t}」……こういう時こそ、筋を通すことだ。${l}で確かめておいで。`,
  ],
  dev: [
