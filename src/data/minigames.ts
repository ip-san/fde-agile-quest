...
// ───────────────────────────────────────────────────────────
// goalcreep 文脈の説得デッキ＝スプリント途中の割り込み交渉（s2-daily-goalcreep）。
// 結城さん（情シス）が経営直々の割り込みを持ち込み、スプリントゴールが揺らぐ場。
//
// 【agile 考証の肝】Scrum: スプリントゴールを危うくする変更は入れない／スコープは PO とディベロッパーが
// 交渉して明確化・再交渉できる。だから「効く」論拠は——拒否ではなく、残キャパシティと完成の定義という
// “事実”で線を引き、入れるなら今ここで何を外すかをセットで決めさせる方向（割り込み＝常に悪ではなく、
// 正式な再交渉に乗せるのが正道）。「効かない」のは——迎合（頼みだからやる）・先送り（期日は後で調整）・
// 全部呑む（特別に全部）。いずれも事実を伴わず、ゴールと完成の定義を黙って溶かす。
//
// 学習設計の注意（learning-designer 観点を先回り）：良手は「断る」一色にしない。3枚とも“割り込みを正式に
// 扱う”が共通核で、強調点が違う（残キャパシティ／差し替えの明示／本来ゴールへの跳ね返り）＝丸暗記でなく、
// 「事実で線を引く」型を読ませる。良手にも機会コストがある——線を引けば、経営に即“やります”と言える
// 手応えは差し出せない（trust を取り逃す側）。この含意は events 側の resultText が引き取る。
// ───────────────────────────────────────────────────────────
const GOALCREEP_GOOD = [
  // 残キャパシティの事実で「両方は完成の定義を割る」と示す
  '今の残キャパシティで両方やると、どちらも{{完成の定義}}を割ります。割れた物はお見せできません',
  // 入れるなら今ここで何を外すかをセットで決めさせる（再交渉を正式に）
  '{{スプリントゴール}}を書き換えるなら、その場で外す項目を一緒に決めましょう。差し替えなら正道です',
  // 本来ゴール（誤出荷削減）への跳ね返りを事実で示す
  'この割り込みを足すと、誤出荷削減という本来のゴールが今スプリントで届かなくなります',
]
const GOALCREEP_BAD = [
  // 迎合：頼みだからやる（事実も線も無い）
  '結城さんの頼みなら、なんとかします。容量は気合いで作りますよ',
  // 先送り：期日は後で調整できる（完成の定義を黙って溶かす）
  '期日は後で調整できますから、ひとまず両方とも今スプリントに入れておきましょう',
  // 全部呑む：今回は特別に（ゴールを無交渉で危うくする）
  '今スプリントは特別に、頼まれたものは全部やってみます',
]

/** goalcreep 文脈の説得デッキ＝スプリント途中の割り込み交渉。残キャパシティ・完成の定義・本来ゴールへの
 *  跳ね返りという“事実”で線を引く論拠が「効く」、迎合・先送り・全部呑むが「効かない」。良手/悪手の混合。 */
export const GOALCREEP_PERSUADE_DECK: HearingOption[] = [
  ...GOALCREEP_GOOD.map((text) => ({ text, good: true })),
  ...GOALCREEP_BAD.map((text) => ({ text, good: false })),
]

/** 説得の文脈別デッキを返す。汎用は温存し、'demo'（お披露目）／'goalcreep'（割り込み交渉）だけ
 *  専用デッキに差し替える。未指定（PO 説得など従来の6デイリー）は汎用 PERSUADE_DECK のまま＝
 *  既存挙動・採点を壊さない。 */
export function persuadeDeckFor(context?: PersuadeContext): HearingOption[] {
  if (context === 'demo') return DEMO_PERSUADE_DECK
  if (context === 'goalcreep') return GOALCREEP_PERSUADE_DECK
  return PERSUADE_DECK
}
...
const REVIEW_CASES: ReviewCase[] = [
  ...
  // (A) 過剰実装：受入条件に無い汎化・予測機能を足す。狙いは「誤出荷に効く一点」。今は過剰＝文脈依存で正解が決まる。
  {
    ...
    options: [
      ...
      {
        text: '予測・発注まで足すと検証もレビューも一気に増える。誤出荷率に効かない作り込みに工数を取られ、本来塞ぐべき一点の確認が薄くなる',
        issue: true,
        kind: 'direction',
      },
...
const DRILL_SETS: RawDrillSet[] = [
  ...
  // 8b. 循環の核心に肉薄する（chousa：契約書×請求書×台帳の三点照合・同じ機材が書類上を一周する）。
  //     不正暴露アークの“もう一段深い裏取り”＝アークが進むほど核心へ寄る手応えを担保する4セット目。
  {
    theme: 'chousa',
    questions: [
      {
        text: 'この納品、契約書・請求書・受領の控え、三つとも突き合わせてみましたか？',
        good: true,
        response:
          '「契約書と請求書は揃ってます。でも…受領の控えだけが、どこにもないんです。モノが入った記録だけが、すっぽり抜けていて」',
        followUps: [
          { text: '受領の控えだけが無いんですね。同じ品番で、過去の取引はどうなっていますか？', good: true },
          {
            text: 'その契約書と請求書、宛先と振出先をたどると、最後はどこの会社に戻ってきますか？',
            good: true,
          },
          {
            // 巧い悪手：返答の「契約書と請求書は揃ってる」を引いて確かめたふりで、現物が動いた証拠（受領）の不在を「紙が揃えば十分」へすり替える
            text: '「契約書と請求書は揃ってる」のなら、紙の上では成立していますよね。受領の控えは事務処理の漏れでしょうから、揃っている二枚を正として進めて大丈夫ですよね',
            good: false,
          },
          {
            text: '控えが一枚無いくらいは、どの取引でもよくある話ですから、後で先方に再発行を頼むことにして、今は揃っている分で帳簿を締めてしまいましょう',
            good: false,
          },
        ],
        finalReactions: {
          onGreat:
            '「（契約書を並べ替えて）…ここ。この機材、半年で四社を一周して、最後にうちへ戻ってきてます。書類は回ってるのに、現物が動いた控えは、一枚もない（声が低くなる）」',
          onMid:
            '「（並べかけた控えを、そろえて戻して）…揃っている二枚で進めていい、と言われれば、確かにそれで締まりますね。受領の控えなんて、出てこない取引、いくらでもありますし。深く考えるほどの話でも…まあ、ないでしょう」（一周の符合を、ありふれた事務漏れに均してかわす）',
          onPoor: '「……（控えをファイルに戻す）。経理に確認しておきます」',
        },
      },
      {
        text: 'この一連の取引、最初に「やれ」と決めたのは誰ですか？',
        good: true,
        response:
          '「…現場じゃないです。本部から「この機材を計上に乗せろ」と指示が来て、私はそれを伝票に起こしただけで。なぜそうするのかは、聞かされていません」',
        followUps: [
          { text: '本部からの指示、口頭でしたか、文書でしたか？ その指示が残っている記録はありますか？', good: true },
          {
            text: '伝票に起こしただけ、ということは——あなたは、その機材を実際に見たことがありますか？',
            good: true,
          },
          {
            text: '本部の指示で動いたなら、判断したのは本部であってあなたではありませんよね。指示通りに処理した記録さえ残っていれば、それ以上たどる必要はないですよね',
            good: false,
          },
          {
            text: '誰が決めたかを掘り出すと社内の話に踏み込みますから、そこは触れずに、出てきた伝票が正しく処理されているかだけ確かめて先へ進みましょう',
            good: false,
          },
        ],
        finalReactions: {
          onGreat:
            '「（メールを探して）…指示は、これです。「期末までに計上を立てること」とだけ。何のために、どこに現物があるかは、一言も書いてない。そして私は——あの機材を、一度も見ていません（目を伏せる）」',
          onMid:
            '「（探しかけた手を止めて）…本部が決めたんだから、私の責任じゃない、と言われれば、そうなんですよね。それで気は楽になります。——現物を見たかどうかは…まあ、見なくても処理は回りますし、わざわざ」（指示の出どころを、責任の切り分けにすり替えて止まる）',
          onPoor: '「……（メールの画面を閉じる）。指示通りにやっただけ、ですから」',
        },
      },
      {
        text: '帳簿の上で辻褄が合っているなら、それで売上として認めていいですよね？',
        good: false,
        response: '「数字の上は、合っています」（「数字の上は」と、わざわざ前置きを置く）',
        followUps: [
          {
            text: '「数字の上は」と前置きされましたね。その売上に対応するモノは、実際にどこかへ動きましたか？',
            good: true,
          },
          { text: '同じ機材が何度も売上に立っていないか、品番でひと通り並べて見せてもらえますか？', good: true },
          {
            text: '会計上の辻褄が合っているなら正規の売上として処理して問題ありませんから、現物がどう動いたかまでは追わずに、数字のまま計上を確定しましょう',
            good: false,
          },
          {
            text: 'システムが弾かずに通している取引なら整合は取れているはずですから、同じ品番が何度立とうと、一件ずつは正しい以上、合計も正しいとして締めましょう',
            good: false,
          },
        ],
        finalReactions: {
          onGreat:
            '「（伝票を品番で並べ直して）…同じ機材が、四回、売上に立ってます。買って、売って、また買って。書類だけが、ぐるぐる回ってる（息をのむ）」',
          onMid:
            '「（並べかけた伝票に、手を添えたまま）…数字が合っているなら正規の売上、と言われれば、一件ずつはどれも正しいんですよね。同じ品番が何度立とうと、一件ごとに契約があるなら…それを疑い出すと、きりがなくて」（一周の輪を、一件ずつの正しさにほどいてかわす）',
          onPoor: '「……（伝票をそろえて戻す）。計上は、このまま通します」',
        },
      },
      {
        text: 'これだけ揃えば、もう循環取引の証拠ですよね？',
        good: false,
        response: '「…そう言い切られると、私の立場では、困ります」（核心に触れられ、急に口が重くなる）',
        followUps: [
          {
            text: '言い切ってしまい失礼しました。証拠と呼ぶ前に、契約書・請求書・受領の控え、揃っている所と欠けている所を一枚ずつ分けて確かめさせてください',
            good: true,
          },
          {
            text: '結論はいったん置きます。確かめられた事実と、伝聞や推測を分けて、辿れる所だけ拾い直しましょう',
            good: true,
          },
          {
            text: 'これだけ符合が揃っているなら、もう循環取引で間違いないでしょうから、細かい裏取りは飛ばして、誰の指示だったかを特定する方へ一気に詰めましょう',
            good: false,
          },
          {
            text: '状況がここまで揃えば証拠は十分ですから、一枚ずつ確かめる手間はかけずに、不正という前提で報告書を先にまとめてしまった方が早いですよね',
            good: false,
          },
        ],
        finalReactions: {
          onGreat:
            '「（少し肩の力が抜けて）…決めつけられると、何も出せなくなるので。一枚ずつ、なら。——一つだけ、確かなことを言います。この機材、契約書には型番があるのに、倉庫の現物台帳には、一度も載ったことがない。最初から、モノは無いんです（声を落として、目を合わせる）」',
          onMid:
            '「（少し身構えが解けて）…一枚ずつ分けてくれる、と言うなら、まだ話せます。ただ…どこまで出していいものか。受領の控えのことを言えば、本部の指示の話まで戻ってしまうし。線引きが、難しくて」（口を開きかけて、また閉じる）',
          onPoor: '「……（ファイルを閉じて、目を伏せる）。この件は、私からは何も」',
        },
      },
    ],
  },
