  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-lg flex-col justify-center gap-6 px-safe py-10">
      ...
      {!failed && (
        <div className="rounded-2xl border border-amber-600/40 bg-amber-950/20 p-5">
          ...
          {/* 不正の伏線を掴んだ周回だけ、個人の”違和感”の引きを重ねる。決着はつけず（§6.5）、
              ...
              掴めなかった周回（fraudHint='none'）も、何かが引っかかる気配だけ仄めかす。 */}
          {teaser ? (
            ...
            <p className="mt-3 border-t border-amber-600/20 pt-3 text-sm leading-relaxed text-amber-100/60">
              ——ふと、田淵さんの口から出た数字と、いつか見た書類の数字が、頭の隅で噛み合わないままだ。気のせいかもしれない。だが、その引っかかりは消えなかった。
            </p>
          )}
        </div>
      )}
      ...
      {!failed && (rank.grade === 'C' || rank.grade === 'D') && (
        <div className="rounded-2xl border border-[var(--border-strong)]/60 bg-[var(--card)]/80 px-5 py-4">
      ...
      <div
        className={phase === 'flash' ? 'invisible' : isHighRank ? 'motion-safe:animate-[rankReveal_0.4s_ease-out]' : ''}
      >
