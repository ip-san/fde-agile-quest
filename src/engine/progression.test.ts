...
import { EVENTS, PRODUCT_BACKLOG, SPRINTS, STARTING_METERS } from '../data/chapters/chapter-01'
...
describe('spinCore', () => {
  ...
  it('出せるイベントが尽きたビートは素通りで次へ進む（予測を組んでプランニングを抜ける）', () => {
    const core: ProgressCore = {
      ...freshCore(STARTING_METERS),
      resolvedIds: new Set(['s1-plan-goal']),
      sprintForecast: [PRODUCT_BACKLOG[0].id], // スプリントバックログを1件組んで関門を通す
    }
...
describe('proceedCore / isRouletteCeremony（単発セレモニーの「進める」）', () => {
  ...
  it('ゴールを決め予測を組んでいれば、進めるでプランニングを抜けて次ビートへ', () => {
    const next = proceedCore({
      ...freshCore(STARTING_METERS),
      resolvedIds: new Set(['s1-plan-goal']),
      sprintForecast: [PRODUCT_BACKLOG[0].id],
    })
    expect(next.status).toBe('playing')
    expect(next.beatIndex).toBe(1)
  })
...
describe('プランニングの関門：空のスプリントバックログでは始められない', () => {
  it('canLeavePlanning は予測が空なら false / 1件以上で true', () => {
    expect(canLeavePlanning({ sprintForecast: [] })).toBe(false)
    expect(canLeavePlanning({ sprintForecast: [PRODUCT_BACKLOG[0].id] })).toBe(true)
  })

  it('ゴール選択(chooseCore)は即・次へ進めず、プランニングに留まる（成果物を作る一手を通す）', () => {
    const s1plan = EVENTS.find((e) => e.id === 's1-plan-goal')
    if (!s1plan) throw new Error('s1-plan-goal not found')
    const next = chooseCore(eventCore({ currentEvent: s1plan, beatIndex: 0 }), s1plan.choices[0])
    expect(next.status).toBe('playing')
    expect(next.beatIndex).toBe(0) // プランニングのまま
    expect(next.sprintGoals[0]).toBeTruthy() // ゴールは決まる
    expect(next.result).not.toBeNull() // 結果は見せる
  })

  it('ゴール決定後でも予測が空なら「進める」は素通り（no-op）', () => {
    const core: ProgressCore = {
      ...freshCore(STARTING_METERS),
      resolvedIds: new Set(['s1-plan-goal']),
      sprintForecast: [],
    }
    expect(proceedCore(core)).toBe(core) // 進まない
  })
})
...
describe('spinCore — 縦糸の入口(pinned)の強制提示', () => {
  ...
  // pinned（S2）: s2-daily-ghost-stock（不正暴露の入口）＋ s2-physical-ai-showcase（フィジカルAIお披露目）。
  ...
  it('最後のデイリーで未遭遇 pinned が残っていれば必ず単独提示する', () => {
    const next = spinCore(at(0, 5), 'kokyaku', 0)
    expect(next.status).toBe('travel')
    expect(next.dailyCandidates).toEqual(['s1-daily-hideknowhow']) // avail 先頭の pinned
  })

  it('複数 pinned は末尾の複数デイリーで全部強制される（残2日・未遭遇2件→順に強制）', () => {
    // beatIndex 4（最後から2日目）: 残りデイリー2・未遭遇 pinned 2件 → 強制が始まる
    const day4 = spinCore(at(0, 4), 'kokyaku', 0)
    expect(day4.dailyCandidates).toEqual(['s1-daily-hideknowhow'])
    // 1件目を消化していれば、最終日に残りの pinned が強制される
    const day5 = spinCore(at(0, 5, { resolvedIds: new Set(['s1-daily-hideknowhow']) }), 'kokyaku', 0)
    expect(day5.dailyCandidates).toEqual(['s1-daily-showcase-order'])
  })

  it('S2: ghost-stock と physical-ai-showcase は末尾2日で必ず通る（showcasePressure 前提）', () => {
    const flags = new Set<GameFlag>(['showcasePressure']) // S1 で必ず立つ前提
    const day4 = spinCore(at(1, 4, { flags }), 'genba', 0)
    expect(day4.dailyCandidates).toEqual(['s2-daily-ghost-stock'])
    const day5 = spinCore(at(1, 5, { flags, resolvedIds: new Set(['s2-daily-ghost-stock']) }), 'genba', 0)
    expect(day5.dailyCandidates).toEqual(['s2-physical-ai-showcase'])
  })

  it('pinned が全て解決済みなら強制せず通常の候補を引く', () => {
    const core = at(0, 5, { resolvedIds: new Set(['s1-daily-hideknowhow', 's1-daily-showcase-order']) })
    const next = spinCore(core, 'kokyaku', 0)
    expect(next.dailyCandidates).not.toContain('s1-daily-hideknowhow')
    expect(next.dailyCandidates).not.toContain('s1-daily-showcase-order')
  })

  it('残りデイリーに余裕がある前半は強制しない（pinnedは最優先候補だが単独固定ではない）', () => {
    const next = spinCore(at(0, 1), 'kokyaku', 0)
    expect(next.status).toBe('travel')
