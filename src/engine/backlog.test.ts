...
describe('reviewItem（レビュー＝In Progress→Done）', () => {
  it('深いレビューはレビュー容量を消費し、進捗とカバレッジを積む', () => {
    ...
    expect(n.reviewCapacity).toBe(2) // 容量4(テスト値)から thorough cost 2 を消費
  ...
  it('浅いレビューは負債を増やす', () => {
    ...
    expect(n.reviewCapacity).toBe(3) // 容量4(テスト値)から quick cost 1 を消費
  })
...
describe('機構：Retro 昇格（レトロ改善が次スプリントに効く）', () => {
  ...
  it('dailyBeatsInSprint: そのスプリントの daily ビート数を返す（S1=5）', () => {
    expect(dailyBeatsInSprint(0)).toBe(5) // chapter-01 の各スプリントは daily×5
    expect(dailyBeatsInSprint(999)).toBe(0) // 存在しないスプリントは 0
  })
  it('reviewBudgetForSprint: 予測の目安＝1日の容量 × デイリー日数（スプリント全体）', () => {
    expect(reviewBudgetForSprint([], 0)).toBe(REVIEW_CAPACITY_PER_DAY * 5) // 日次×5日
    expect(reviewBudgetForSprint(['capacity'], 0)).toBe((REVIEW_CAPACITY_PER_DAY + 1) * 5) // capacity改善も乗る
  })
  ...
  it('daily ビートに進入すると reviewCapacity が日次リセットされ、capacity レバーが即時反映される', () => {
    ...
    // 次スプリント最初の daily 進入時に reviewCapacityFor(['capacity'])=3（=2+1）へリセットされる（即時反映）。
    ...
    expect(firstDaily.reviewCapacity).toBe(REVIEW_CAPACITY_PER_DAY + 1) // 2 + capacity1 = 3（日次リセット＋即時反映）
  })

  it('daily→daily の進行でも毎日リセットされ、前日の残容量は繰り越さない（使い切り）', () => {
    // daily（beatIndex=1）で消費途中（残1）→ 次の daily（beatIndex=2）へ進入すると、残1は破棄され満タンへ戻る。
    const dailyCore = core({
      sprintIndex: 0,
      beatIndex: 1, // daily ビート
      retroImprovements: [], // レバー無し＝日次2
      reviewCapacity: 1, // 消費途中（繰り越さない）
      status: 'playing',
    })
    const next = advanceCore(dailyCore)
    expect(next.sprintIndex).toBe(0)
    expect(next.beatIndex).toBe(2) // 次の daily
    expect(next.reviewCapacity).toBe(REVIEW_CAPACITY_PER_DAY) // 残1は繰り越さず日次2へリセット
  })
...
describe('永続化ラウンドトリップ / 旧セーブ復元', () => {
  ...
  it('カンバン状態（inProgress/reviewProgress/reviewCapacity）も往復で保たれる', () => {
    const c = core({
      ...
      reviewCapacity: 1, // 日次上限(REVIEW_CAPACITY_PER_DAY=2)以内＝そのまま往復で保たれる
    })
    const round = restoreCore(toPersisted(c))
    expect(round.inProgress).toEqual([ID.floor])
    expect(round.reviewProgress).toEqual({ [ID.floor]: 2 })
    expect(round.reviewCapacity).toBe(1)
  })
