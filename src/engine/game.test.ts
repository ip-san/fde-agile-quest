...
describe('リポジトリの健全度（拡充＝量と質を伴う）', () => {
  ...
  it('丸投げ（aiOverreliance）の選択は技術的負債を積む', () => {
    for (const e of EVENTS) {
      for (const c of e.choices) {
        if (c.setsFlag === 'aiOverreliance') {
          expect((c.repo?.debt ?? 0) > 0, `${e.id}/${c.id} 丸投げなのに負債を積まない`).toBe(true)
        }
      }
    }
  })

  it('repo.coverage/debt は妥当な範囲の整数', () => {
    for (const { e, c } of repoChoices) {
      const cov = c.repo?.coverage ?? 0
      const debt = c.repo?.debt ?? 0
      expect(Number.isInteger(cov) && Number.isInteger(debt), `${e.id}/${c.id}`).toBe(true)
      expect(Math.abs(cov), `${e.id}/${c.id} coverage 過大`).toBeLessThanOrEqual(30)
      expect(Math.abs(debt), `${e.id}/${c.id} debt 過大`).toBeLessThanOrEqual(3)
    }
  })
})
