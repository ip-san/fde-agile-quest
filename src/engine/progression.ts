 ...
 *  日次レビュー容量を“絞った”後（REVIEW_CAPACITY_PER_DAY=2 × daily5 × 3sprint ＝ 理論30pt前進力／
 *  素good・負債ドラッグ込みで実効20-25pt 前後）でも、PB全14件中の見積りは小粒（2-5pt）が多く、強いプレイ
 *  （great1.5倍・quickで広く・スコープ再交渉での引き込み）なら 8-10件程度は Ship しうる帯。
 *  9 は「強いプレイで届けきれるが、全完遂ではない」ラインで、終盤まで delivery 項の伸びを残す狙い。
 *  ※容量を 4→2 に半減した直後なので、9 が妥当かは監修プレイテストで実測・再調整の余地あり（要観察）。 */
